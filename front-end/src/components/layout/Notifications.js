...
class Notifications extends Component {
    ...
    onMenuOpened = () => {
        let unreadNotificationsIDs = this.props.notifications.filter(notif => notif.read)
    ...
    render(){
        ...
        if( notifications && notifications.length > 0 ){
            notifications.filter(notification =>  notification.read === false).length > 0 ? notificationsIcon = (
        ...
        let notificationsMarkup = notifications && notifications.length > 0 ? (
            notifications.map(notif => {
                ...
                const icon = notif.type === 'like' ? ( <FavoriteIcon color={iconColor} style={{marginRight: 10}} />
                ) : ( <ChatIcon color={iconColor} style={{ marginRight: 10 }} />)

                return(
                    <MenuItem key={notif.createdAt} onClick={this.handleClose} >
                        {icon}
                        <Typography component={Link} color='default' variant='body1' to={`/users/${notif.recipient}/shout/${notif.shoutID}`} >
